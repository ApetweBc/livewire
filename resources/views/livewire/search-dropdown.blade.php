...
<div>
  ...
  <div>
      <input wire:model.debounce.300ms="search" type="search" class="w-full pl-3 border-solid border border-gray-300" >
  </div>
 ...
 @if (strlen($search) > 3 )
   ...
   <a href="{{$result['trackViewUrl'] ?? '#'}} " target="_self">
   <div class="pl-5 mb-3 mb-4">
    <img src="{{$result ['artworkUrl60']}}" class="img-fluid ${3|rounded-top,rounded-right,rounded-bottom,rounded-left,rounded-circle,|}" alt="">
    <ul class="list-none">
        <li> {{$result['artistName'] ?? 'Artist Name is  not Vailable'}}</li>
        <li>{{$result['trackName'] ?? 'Track Name not found'}} </li>
        <li>{{$result['releaseDate'] ?? 'Realeased Date not Available'}}</li>
    </ul>
    </div>
    @empty
